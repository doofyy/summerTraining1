...
$result2 = mysqli_query($mysqli , $query); ?>
    ...
    <td>
	  <label>
          <button type="submit"  name="options0" value="emp_id"  > Employee </button>
      </label>
    </td>

    <td>
	  <label>
          <button type="submit"  name="options0" value="course"    > Course </button>
      </label>
    </td>
  <?php
  if(isset($_POST["options0"])){

    $_POST['flag'] = $_POST["options0"];
...
<?php
 ...
 if(isset($_POST["submit"]) && isset($_POST["svar"])  && isset($flag)){
   $print = "";
   echo $_POST['flag'];
   die();
   $checkquery = "Select course from mids where ". $test ." = " . '"' . $_POST["svar"] . '"';
   ...
   $check = mysqli_num_rows(mysqli_query($mysqli,$checkquery));
    ...
    if($_POST["options" ] == "average" ){
        ...
        if($_POST["svar"] != "All" && $check){
          $query = $query. " where " . $flag ." = ". '"' . $_POST["svar"] . '"';
        }
